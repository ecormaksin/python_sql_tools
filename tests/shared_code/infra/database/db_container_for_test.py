...
from shared_code.infra.database.sqlalchemy_sql_file_executor import (
    SQLAlchemySQLFileExecutor,
)
from tests.project_root_resolver import ProjectRootResolver
from tests.shared_code.infra.database.ddl_file_name_control_entity import (
    DDLFileNameControlEntityList,
)
...
class DbContainerForTest:
    def __init__(self, rdbms_type: RDBMSType):
        ...
        db_container = rdbms_type_for_test.db_container

        if not db_container:
            raise RuntimeError(
                f"'{rdbms_type.key}' に対応するRDBMSの種類を指定してください。"
            )
        ...
        original_db_connection = original_db_engine.connect()
        self._original_db_connection = original_db_connection

        for serial_number, ctrl_entity in enumerate(
            DDLFileNameControlEntityList.get(), start=1
        ):
            ...
            if rdbms_type == RDBMSType.MYSQL:
                url = re.sub("/test$", f"/{schema_name}", url)
            scheme_replaced_url = rdbms_type.replace_scheme(url=url)

            if rdbms_type_for_test.check_database_exists(
                url=scheme_replaced_url,
                connection=original_db_connection,
                schema_name=schema_name,
            ):
                schema_ddl_file = schema_ddl_dir.joinpath(ddl_file_name)
                SQLAlchemySQLFileExecutor.execute(
                    file_path=schema_ddl_file, connection=original_db_connection
                )
            ...
            with engine.connect() as db_connection:
                ...
                SQLAlchemySQLFileExecutor.execute(
                    file_path=table_ddl_file, connection=db_connection
                )
                ...
                if ctrl_entity.view:
                    ...
                    SQLAlchemySQLFileExecutor.execute(
                        file_path=view_ddl_file, connection=db_connection
                    )
    ...
    def __exit__(self, exc_type, exc_value, a_traceback):
        if self._original_db_connection:
            self._original_db_connection.close()
